...
func apply_effect(effect: StatusEffect, source: Character) -> void:
	var effect_array: Array = effects.get_or_add(effect.get_name(), [])
	
	if effect_array.is_empty():
		_add_effect(effect, source, effect_array)
		return
...
func _add_effect(effect: StatusEffect, source: Character, effect_array: Array) -> void:
...
func tick(delta: float) -> void:
	for effect_array: Array in effects.values():
